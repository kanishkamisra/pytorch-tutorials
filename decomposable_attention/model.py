...
class Decomp(nn.Module):
  def __init__(self, **kwargs):
    ...
    self.linear_decide = nn.Sequential(nn.Linear(self.emb_size*2, self.hidden_size), nn.ReLU(), nn.Linear(self.hidden_size, self.output_size))

  def forward(self, sentence1, sentence2):
    sent1, send2 = [self.embed(x) for x in [sentence1, sentence2]]

    transformed1, transformed2 = [linear_transform(x) for x in [sent1, sent2]]
    ...
    paired1, paired2 = [linear_pair(x) for x in [concatenated1, concatenated2]]

    v1, v2 = [torch.sum(x, 0) for x in [paired1, paired2]]

    pred = F.log_softmax(linear_decide(torch.cat((v1, v2), 1)))
